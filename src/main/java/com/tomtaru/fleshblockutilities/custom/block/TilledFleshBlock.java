...
public class TilledFleshBlock extends FarmBlock {
    ...
    private static boolean isNearBlood(LevelReader level, BlockPos pos) {
        for (BlockPos blockpos : BlockPos.betweenClosed(pos.offset(-4, 0, -4), pos.offset(4, 1, 4))) {
            ...
            if ((currentState.is(BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath("biomesoplenty","blood")))
            || currentState.is(BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath("biomesoplenty", "flowing_blood"))))
            && !currentState.isAir()) {
